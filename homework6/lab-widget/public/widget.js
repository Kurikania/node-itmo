...
var wrgsv = {
  ...
  init: function () { /*функция инициализации виджета */
    if (document.getElementById(this.idBox)) { /*проверяем наличие в DOM элемента с указанным идентификатором */
      ...
      }; /* вешаем обработчик ошибок получения данных от сервера */
      xhr.send(); /* отправляем запрос на сервер */
    } else {
      console.log('The specified block ID "' + id + '" is missing'); 
      /* выводим сообщение об отсутствие в DOM тега с указанным id */
    }
  },

  addStyle: function () {
    /* метод формирует тег link и вставляет его в документ */
  ...
  addWidget: function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      /*для каждого объекта из массива данных с сервера */
