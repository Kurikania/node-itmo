...
/* Объект используется для хранения в виде полей cookies пользователей, которые прошли авторизовались и имеют доступ к панели администратора */
let authCookies = {};
...
route.use((req, res, next) => {
  ...
  if (req.originalUrl === "/admin") {
    ...
    /*Пользователь не запрашивает панель администратора, поэтому проверяем есть ли cookies разрешающая доступ к этой части сайта*/
    let sid = req.cookies.sid; //sid – название cookies отвечающей за доступ на нашем сайте
    if (sid && authCookies[sid]) {
      //проверяем наличие cookies и наличие разрешения
      next(); //разрешение есть, пропускаем запрос дальше
    } else {
      res.redirect(
        "/admin"
      ); /*в случае не знакомого пользователя перенаправляем клиента на страницу /admin */
    }
  }
});
...
/*Роут организующий прием данных со странички аутентификации. Полный путь: /admin/. Внутри осуществляется проверка логина и пароля. Если проверка пройдена, в ответном запросе устанавливаем специальную cookies и перенаправляем на страницу /admin. Если проверка не пройдена возвращаем страницу аутентификации с сообщением об ошибке */
route.post("/", (req, res, next) => {
  ...
  db.findUser(req.body.login, (err, user) => { //Проверяем наличие в хранилище пользователя
    if (!user) //Нет такого пользователя
      /*функция рендер наполняет шаблон (файл auth.mustache) данными указанными вторым аргументом (вернется сообщение об ошибке)*/
      return res.render("auth", { message: true });
    
    let passwordFromClient = crypto //Шифруем присланный пароль
      .createHash("sha512")
      .update("salt" + req.body.pass)
      .digest("hex");
    
    if (user.password !== passwordFromClient) //Сравниваем шифрованный присланный пароль с хранящемся на сервере
      //Неверный пароль
      /*функция рендер наполняет шаблон (файл auth.mustache) данными указанными вторым аргументом (вернется сообщение об ошибке)*/
      return res.render("auth", { message: true });
    ...
    let token = crypto
      ...
      .digest("hex");
    authCookies[token] = true; /*сохраняем себе пометку, что с этим cookie есть разрешение на доступ к этой части сайта */
...
/*Роут организующий прием запрос о выходе текущего пользователя из системы. Полный путь: /admin/out. Внутри осуществляется удаление ранее установленной cookie и удаление пометки, разрешающей пользователям с этой cookie иметь доступ к этой части сайта*/
