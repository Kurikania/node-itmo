...
http.createServer((request, response) => {
    ...
    if (extname === ".mp4" || extname === ".mp3") {
      if (!fs.existsSync(pathname)) {
        //Функция existsSync проверяет существование запрашиваемого файла
      ...
      if (rangeRequest == null) {
        //Заголовка Range не было в запросе от браузера, а значит сразу полностью возвращаем //файл
      ...
      let maxsize = 10240; /*Максимальный размер частичного контента 10KB (это значение выбрано для примера, может быть задано другое из соображений производительности)*/
      ...
      responseHeaders["Content-Range"] = "bytes " + start + "-" + end + "/" + stat.size; /*Отправляемый диапазон с байтами */
      ...
      /*Читаем часть файла. Вызываем функцию чтения со следующими аргументами: fd – дискриптор файла, buf – буфер куда сохранить почитанные байты, 0 – позиция с которой надо писать в буфер, buf.length – количество байтов для прочтения из файла, start – это позция в файле с которой читать байты */
